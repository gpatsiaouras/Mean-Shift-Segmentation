...
class Segmentation:
    ...
    def mean_shift(self):
        ...
        sys.stdout.write("\rTime took: {0:.2f} seconds".format(time.time() - start_time) * 1000)
        sys.stdout.flush()

    def get_data_in_radius_from_point(self, cluster_point):
        condition = np.linalg.norm(cluster_point - self.data, axis=0) < self.radius
        extracted = np.extract(np.tile(condition, (3, 1)), self.data)
        return extracted.reshape(3, extracted.shape[0] // 3)
